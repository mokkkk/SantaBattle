...
# 動物を召喚
    execute store result score #temp_random temp run random value 0..4
...
# ステータス設定
    execute store result score #temp_random temp run random value 10..150
    ...
    execute store result score #temp_random temp run random value 800..1800
    scoreboard players operation @s player.sled_status.max_speed = #temp_random temp
    execute store result score #temp_random temp run random value 10..60
    scoreboard players operation @s player.sled_status.acceleration = #temp_random temp
    execute store result score #temp_random temp run random value 10..100
    scoreboard players operation @s player.sled_status.attack_deceleration = #temp_random temp
    execute store result score #temp_random temp run random value 3000..6500
